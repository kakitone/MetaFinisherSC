...
def useMummerAlign(mummerLink, folderName, outputName, referenceName, queryName, fineGrain = False):
    
    if fineGrain == True:
        command = mummerLink + "nucmer --maxmatch -l 10 -p " + folderName + outputName + " " + folderName + referenceName + " " + folderName + queryName
    else:
...
class seqGraph(object):
    ...
    def reportEdge(self):
        for eachnode in self.graphNodesList:
            myName = self.formReportName(eachnode.nodeIndex) 
            if len(eachnode.nodeIndexList) > 0:
                for eachnextnode in eachnode.listOfNextNodes:
                    nextName = self.formReportName(eachnextnode[0])
                    wt = eachnextnode[1]
                    if wt > 1000:
                        print myName + "->" + nextName + "{weight:" + str(int(wt / 1000)) + "}"
                    else:
                        print myName + "->" + nextName + "{weight:" + str(0.5) + "}"
                        
        for eachnode in self.graphNodesList:
            myName = self.formReportName(eachnode.nodeIndex) 
            if len(eachnode.nodeIndexList) > 0:
                if len(eachnode.listOfNextNodes) == 0 and len(eachnode.listOfPrevNodes) == 0:
                    print myName + "{color:red}"
...
def compareWithReference(folderName , mummerLink):
    ...
    quastEvaluate(folderName, "quast-2.3/", originalName="contigs.fasta", improvedNameList=["improved2.fasta"] , referenceName="reference.fasta")
